...
const InputForm: Component<Props> = props => {
  const [mPlayers, setMPlayers] = createSignal<string[]>([]);
  const [fPlayers, setFPlayers] = createSignal<string[]>([]);
  ...
  const handleMPlayersChange = (value: string) => {
    setMPlayers(value.split("\n").map(x => x.trim()).filter(Boolean));
  };

  const handleFPlayersChange = (value: string) => {
    setFPlayers(value.split("\n").map(x => x.trim()).filter(Boolean));
  };
  ...
  return <>
    ...
    <p>
      아래 명단에 이름을 <b>실력순으로</b> 작성해주세요!
      제일 위에 있는 사람이 제일 실력이 좋은 사람입니다.
    </p>

    <div class="half">
      <div>
        <label>
          남자 ({mPlayers().length} 명)
          <TextArea
            placeholder={"홍길동\n김철수\n이영희"}
            onValue={handleMPlayersChange}
            />
        </label>
      </div>
      <div>
        <label>
          여자 ({fPlayers().length} 명)
          <TextArea
            placeholder={"홍길동\n김철수\n이영희"}
            onValue={handleFPlayersChange}
            />
        </label>
      </div>
    </div>
    ...
    <div class="half">
      ...
      <label>
        남복 즐겜 수
        <input ref={mFunGamesRef!} type="number" value="0" onChange={handleUpdateGames} />
      </label>

      <label>
        여복 즐겜 수
        <input ref={fFunGamesRef!} type="number" value="0" onChange={handleUpdateGames} />
      </label>
    ...
    <div class="half">
      <label>
        남복 빡겜 수
        <input ref={mHardGamesRef!} type="number" value="0" onChange={handleUpdateGames} />
      </label>

      <label>
        여복 빡겜 수
        <input ref={fHardGamesRef!} type="number" value="0" onChange={handleUpdateGames} />
      </label>
    ...
    <label>
        혼복 즐겜 수
        <input ref={xFunGamesRef!} type="number" value="0" onChange={handleUpdateGames} />
      </label>

      <label>
        혼복 빡겜 수
        <input ref={xHardGamesRef!} type="number" value="0" onChange={handleUpdateGames} />
      </label>
